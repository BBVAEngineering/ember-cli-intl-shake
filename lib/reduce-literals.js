...
function cleanModules(modules) {
	return Object.entries(modules).reduce((acc, [module, literals]) => {
		...
		literals = literals.filter((literal) => literal.match(/^[\w-.]+$/));
...
function getModules(files) {
	return files.reduce((acc, [literals, relativePath]) => {
		const [, module] = relativePath.match(/^([^/]+)/);
...
module.exports = class ReduceLiterals extends CachingWriter {
	...
	extractCommon(modules) {
		const commonLiterals = modules[this.common];
		const sideModules = Object.entries(modules)
			.filter(([module]) => module !== this.common)
			.map(([module, literals]) => [
				module,
				literals.filter((literal) => !commonLiterals.includes(literal))
			])
			.reduce((acc, [module, literals]) => {
				acc[module] = literals;
	...
	async build() {
		...
		let modules = getModules(files);

		this.debug('cleaning modules...');

		modules = cleanModules(modules);

		this.debug('extracting common literals...');

		modules = this.extractCommon(modules);
