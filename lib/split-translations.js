  ...
  extractPairs(changes, entries) {
    ...
    const pairs = changes.reduce((acc, change) => {
      if (change.match(this.isLiteral)) {
        ...
        const moduleTranslations = this.getTranslationsByModule(
          translations,
          module
        );

        acc.push(
          ...moduleTranslations.map((translation) =>
            [change, translation].join(':')
          )
        );
      } else {
        ...
        acc.push(
          ...moduleLiterals.map((literal) => [literal, change].join(':'))
        );
      }
